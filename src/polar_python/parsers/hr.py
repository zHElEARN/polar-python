...
def parse_hr_data(data: bytearray) -> HRData:
    ...
    try:
        ...
        rr_intervals = [int.from_bytes(data[i : i + 2], byteorder="little", signed=False) / 1024.0 * 1000.0 for i in range(2, len(data), 2)]
        return HRData(heartrate, rr_intervals)
    except IndexError as e:
        raise ValueError("Failed to parse heart rate data: insufficient data length") from e
